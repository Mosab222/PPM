...
// APPROVED maintenance log's result -- a pending or rejected log's claimed
// result must not affect this, since the whole point of the approval chain
// is verifying that claim before it counts. Computed fresh on every read,
...
export function classifyOperationalStatus(params: {
  ...
  latestCompletedResult: string | null; // caller must pre-filter to approval_status = 'approved'
}): OperationalStatus {
